...
class _OfflineSettingsPageState extends State<OfflineSettingsPage> {
  ...
  final ConnectivityService _connectivityService = Modular.get<ConnectivityService>();
      ...
      if (mounted) {
        ...
        switch (status) {
          case SyncStatus.synced:
            _feedbackService.showSuccessToast(context, 'Sincronização concluída!');
            break;
          case SyncStatus.syncError:
            _feedbackService.showErrorToast(context, 'Erro na sincronização');
            break;
          case SyncStatus.partialSync:
            _feedbackService.showWarningToast(context, 'Sincronização parcial');
            break;
          default:
            break;
        }
  ...
  Future<void> _forcSync() async {
    if (!_isOnline) {
      _feedbackService.showErrorToast(context, 'Dispositivo offline - não é possível sincronizar');
      return;
    }
      ...
      await _feedbackService.executeWithFeedback(
        ...
        () async {
          ...
          if (success) {
            ...
            _feedbackService.showErrorToast(context, 'Falha na sincronização');
          }
        },
      );
    } catch (e) {
  ...
  Future<void> _clearCache() async {
    final confirmed = await _feedbackService.showConfirmationDialog(
      context,
      'Limpar Cache',
      ...
      try {
        await _feedbackService.executeWithFeedback(
          ...
          () async {
            ...
            if (success) {
              _feedbackService.showSuccessToast(context, 'Cache limpo com sucesso!');
              await _loadOfflineData();
            } else {
              _feedbackService.showErrorToast(context, 'Erro ao limpar cache');
            }
          },
        );
      } catch (e) {
  ...
  Future<void> _clearExpiredData() async {
    try {
      ...
      _feedbackService.showErrorToast(context, 'Erro ao limpar dados expirados');
    }
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildStatusItem(
              'Serviços',
              _syncService.isInitialized ? 'Inicializados' : 'Não inicializados',
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text(_isSyncing ? 'Sincronizando...' : 'Forçar Sincronização'),
  ...
  Widget _buildConnectivityCard() {
    final connectivity = _syncStats['connectivity'] as Map<String, dynamic>? ?? {};
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            _buildInfoItem('Tipo de Conexão', connectivity['connectionType'] ?? 'Desconhecido'),
            _buildInfoItem('Status', connectivity['isOnline'] == true ? 'Online' : 'Offline'),
            _buildInfoItem('Última Verificação', _formatTimestamp(connectivity['timestamp'])),
          ],
        ),
      ),
    );
